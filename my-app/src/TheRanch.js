...
function TheRanch ( {bank, setBank, setSpeed, speed}) {
    ...
    return (
        <div>
            <h1>Debt to Bank: ${remainder > 0 ? remainder.toLocaleString(undefined, {maximumFractionDigits:2}) : alert(`You saved Codee Ranch and restored the family name!!!!! You green thumb, you!`)}</h1>
            <h1>Bank Amount: ${bank.toFixed(2).toLocaleString(undefined, {maximumFractionDigits:2})}</h1>
